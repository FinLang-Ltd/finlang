...
_CSS = (
    ...
    ".orphan-note{color:#789;font-size:.85em;margin:.2rem 0 0}"
...
# ---------------------------------------------------------------------------
# Orphans (SOL-104 key-mode follow-on) — rows present on only one side
# ---------------------------------------------------------------------------

def _build_orphan_row(o: Dict[str, str]) -> str:
    """One `<tr>` for an orphan row. Every value html-escaped; memo (if any)
    shown as a muted subline under the counterparty."""
    row_number = html.escape(str(o.get("row_number", "")))
    counterparty = html.escape(o.get("counterparty", ""))
    memo = o.get("memo", "")
    cp_cell = counterparty
    if memo:
        cp_cell += f'<br><span class="meta">"{html.escape(memo)}"</span>'
    return (
        f"<tr><td>{row_number}</td><td>{cp_cell}</td>"
        f'<td>{html.escape(o.get("date", ""))}</td>'
        f'<td>{html.escape(o.get("amount", ""))}</td>'
        f'<td>{html.escape(o.get("category", ""))}</td></tr>'
    )


def _orphans_section(orphans, heading: str, note: str, filename: str) -> str:
    """Build one orphans table section, or "" when there are none.

    Rows built as list-of-strings then joined (O(N), same discipline as the
    mismatch table). `row_number` is the row's position in its OWN file.
    """
    if not orphans:
        return ""
    body = "".join(_build_orphan_row(o) for o in orphans)
    return (
        f"<h2>{heading} ({len(orphans)})</h2>"
        f'<p class="orphan-note">{note} Full set in <code>{html.escape(filename)}</code>.</p>'
        f"<table><thead><tr><th>#</th><th>Counterparty</th><th>Date</th>"
        f"<th>Amount</th><th>Category</th></tr></thead><tbody>{body}</tbody></table>"
    )
...
def generate_html_report(result: ReconcileResult, output_path: str) -> None:
    ...
    # Orphans (SOL-104 key mode) — counts for the meta/banner, sections below.
    n_orphans_fl = len(result.orphans_finlang)
    n_orphans_ml = len(result.orphans_ml)
    orphan_meta = (f" · orphans: {n_orphans_fl} FinLang / {n_orphans_ml} ML"
                   if (n_orphans_fl or n_orphans_ml) else "")
    orphan_banner = (f' · <span style="color:#d34">{n_orphans_fl + n_orphans_ml} '
                     f"orphan rows</span>" if (n_orphans_fl or n_orphans_ml) else "")
    orphans_fl_html = _orphans_section(
        result.orphans_finlang, "Orphans — FinLang rows with no ML match",
        "Rows FinLang categorised that the ML output does not contain — "
        "dropped, unscored, or filtered upstream.",
        "reconcile_orphans_finlang.csv")
    orphans_ml_html = _orphans_section(
        result.orphans_ml, "Orphans — ML rows with no FinLang match",
        "Rows in the ML output that FinLang never produced.",
...
<div class="meta">{timestamp} · {alignment} alignment · field: <code>{fields_str}</code> · finlang {__version__} · audit_entries_loaded={result.audit_entries_loaded}{orphan_meta}</div>
<div class="{banner_class}"><strong>Status: {status_text}</strong> · {result.matches}/{result.rows_compared} match ({match_rate}%) · {mismatch_summary}{orphan_banner} · perfect_match=<code>{perfect_match_str}</code></div>
<table>
