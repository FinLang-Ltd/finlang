...
| `test_impact.py` | 13 impact (standalone — NOT part of `quick_check.ps1`'s 151/10; gate wiring pending explicit Rule-4 approval, test_api.py precedent) | SOL-105 impact analysis: identical packs → exit 0; category/flag/status changes → behavioural exit 3; rename + shadow → attribution-only exit 0; rule removal; hand-computed amount aggregation (150.00); exit-code contract; pass isolation (leak-detector rule); flag validations; JSON schema + hash-equality semantics; CSV column contract + sort + change_class. Run via `python -m pytest test_impact.py` from `test-suite/`. Wall-clock ~25s. | Impact diff semantics, JSON schema changes, CSV column changes, new impact flags, exit-code mapping changes |
