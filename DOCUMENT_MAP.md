...
5. **Validation Infrastructure** — 177-test daily suite (10 gates) + rulepack linter, integrity verification, cleanroom PyPI validation, golden master baselines, ML reconciliation (JSON+CSV+HTML), rule-change impact analysis (JSON+CSV+HTML)
...
| File | Contains | Update When |
|------|----------|-------------|
| `quick_check.ps1` | Daily gate runner (10 gates, 177 tests + rulepack linter), single-line-per-gate display. Gate 10 ("Validation: reconcile + impact") runs `test_reconcile.py` + `test_impact.py` together. Wall-clock ~120–170s typical (varies with cache state). | New daily gates, test count changes |
