...
| `test_reconcile.py` | 33 reconcile (Gate 10) | `--reconcile` ML validation: 12 happy/edge tests + 3 validation rejection tests + 1 verify+reconcile coexistence regression + 1 memo-enrichment dict assertion + 1 HTML render+escape end-to-end + 1 dual-flag validation + 5 identity-guard (SOL-103: pass-through w/ normalised amounts, misalignment exit 1 + artefacts + suppression, validation rejections, baseline-unchanged) + 9 key-alignment (SOL-104: reorder survival, shuffled-row attribution, both orphan sides, duplicate-key strict fail, missing key field, mutual exclusion, validations). Subprocess-based plus 2 module-level direct-API tests. Wall-clock ~40–110s. | Reconcile logic changes, CLI argparse changes, new validation rules, new exit-code paths, mismatch dict shape changes, HTML report format changes, identity/key canonicalisation changes |
| `test_impact.py` | 16 impact (standalone — NOT part of `quick_check.ps1`'s 151/10; gate wiring pending explicit Rule-4 approval, test_api.py precedent) | SOL-105 impact analysis: identical packs → exit 0; category/flag/status changes → behavioural exit 3; rename + shadow → attribution-only exit 0; rule removal; hand-computed amount aggregation (150.00); exit-code contract; pass isolation (leak-detector rule); flag validations (incl. `--impact-html` requires `--impact-rules`); JSON schema + hash-equality semantics; CSV column contract + sort + change_class; **HTML render + escape + float disclaimer (test 14), 50-row cap + CSV-keeps-all (test 15), green banner on no-change (test 16)**. Run via `python -m pytest test_impact.py` from `test-suite/`. Wall-clock ~30s. | Impact diff semantics, JSON schema changes, CSV column changes, new impact flags, HTML report changes, exit-code mapping changes |
