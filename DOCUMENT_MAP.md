...
5. **Validation Infrastructure** — 185-test daily suite (10 gates) + rulepack linter, integrity verification, cleanroom PyPI validation, golden master baselines, ML reconciliation (JSON+CSV+HTML), rule-change impact analysis (JSON+CSV+HTML)
...
| File | Contains | Update When |
|------|----------|-------------|
| `quick_check.ps1` | Daily gate runner (10 gates, 185 tests + rulepack linter), single-line-per-gate display. Gate 10 ("Validation: reconcile + impact") runs `test_reconcile.py` + `test_impact.py` together. Wall-clock ~120–170s typical (varies with cache state). | New daily gates, test count changes |
...
| `test_verify.py` | 16 verify | Integrity verification: --verify (fast), --verify-full, --verify-output-dir, tampered output detection; 4-Jul hardening: injection-quote round-trip (fast+full), blank-amount no-crash, round-trip invariant x2 (both FINLANG_SAFE_TEXT modes); SOL-110 vectorisation: pinned byte-equivalence, scalar/vector unquote property, hash-collision guard | Verification logic or CLI integration changes |
