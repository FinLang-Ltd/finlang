...
| `src/finlang/api/main.py` | FastAPI wrapper around the FinLang CLI (SOL-041): thin REST surface (`GET /`, `GET /health`, `POST /process`, `POST /discover`, `POST /suggest`, `POST /reconcile`) over the published CLI entry points via subprocess. Never imports engine internals. `/reconcile` exit-code semantics: whitelist `{0, 3}` → HTTP 200 (mismatches are an expected review outcome), `1` → 500, `2` → 422. `/reconcile` supports a `?format=html` query param that returns the HTML report directly with `Content-Type: text/html` (bypasses JSON wrapping; requires `reconcile_html=true`); default `format=json` returns the full `ReconcileResponse`. Optional API-key auth via `FINLANG_API_KEY` env var + `X-API-Key` header. Curated, not auto-forwarding — new CLI flags need explicit `Form()` parameters. | New endpoints, new form-field parameters, subprocess dispatch logic changes, auth or limit configuration changes, new response models |
...
| `test_api.py` | 17 API (standalone gate — NOT part of `quick_check.ps1`'s 137/10) | SOL-041 FastAPI wrapper integration tests via `fastapi.testclient.TestClient`: health, root, `/process` (rules + include_pack + verify + validation), `/discover` (default `return_all=False` path), `/suggest`, `/reconcile` (perfect match, mismatches → 200, HTML emission, `format=html` direct-response with `text/html` content-type, `format=html` validation when `reconcile_html=false`), CLI/API reconcile parity contract, auth gating. Run via `python -m pytest test_api.py -v` from `test-suite/` with `pip install -e ../prod[api]` + `pip install httpx` in `.venv-quickcheck`. Wall-clock ~24s. | New API endpoints, form-field changes, response-shape changes, auth behaviour changes, exit-code mapping changes |
