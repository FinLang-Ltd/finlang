...
*Last updated: 16 July 2026 | v0.8.1 (shipped 16 Jul 2026) + SOL-110 verify vectorisation (Path B, unreleased) | reconcile-v2 (SOL-103 identity guard `--reconcile-identity-fields`, SOL-104 key alignment `--reconcile-key` + orphan artefacts) + impact analysis (SOL-105 `--impact-rules`) + HTTP API parity (`/reconcile` identity/key params + orphan CSVs, new `POST /impact`). Daily quick_check 187/10 (Gate 8 = verify, 18 tests; Gate 10 = reconcile + impact, 55 tests); standalone API gate 26. Builds on v0.7.9 SOL-041 FastAPI wrapper (shipped 18 May 2026).*
...
5. **Validation Infrastructure** — 187-test daily suite (10 gates) + rulepack linter, integrity verification, cleanroom PyPI validation, golden master baselines, ML reconciliation (JSON+CSV+HTML), rule-change impact analysis (JSON+CSV+HTML)
...
| File | Contains | Update When |
|------|----------|-------------|
| `quick_check.ps1` | Daily gate runner (10 gates, 187 tests + rulepack linter), single-line-per-gate display. Gate 10 ("Validation: reconcile + impact") runs `test_reconcile.py` + `test_impact.py` together. Wall-clock ~120–170s typical (varies with cache state). | New daily gates, test count changes |
...
| `test_rule_interactions.py` | 26 engine | Cache invalidation, exclude lifecycle, flag accumulation, audit modes, idempotency; 4-Jul hardening: audit `changes` key-order canonical across processes, lite-cap counts logged entries, wildcard `\Z` anchor, `list_rulepacks` `.fin` filter | Engine state-transition changes |
...
| `test_verify.py` | 18 verify | Integrity verification: --verify (fast), --verify-full, --verify-output-dir, tampered output detection; 4-Jul hardening: injection-quote round-trip (fast+full), blank-amount no-crash, round-trip invariant x2 (both FINLANG_SAFE_TEXT modes); SOL-110 vectorisation: pinned byte-equivalence, scalar/vector unquote property, hash-collision guard, reader equivalence + path selection, fast-path liveness (Codex P1) | Verification logic or CLI integration changes |
| `test_reconcile.py` | 38 reconcile (Gate 10) | `--reconcile` ML validation: 12 happy/edge tests + 3 validation rejection tests + 1 verify+reconcile coexistence regression + 1 memo-enrichment dict assertion + 1 HTML render+escape end-to-end + 1 dual-flag validation + 5 identity-guard (SOL-103: pass-through w/ normalised amounts, misalignment exit 1 + artefacts + suppression, validation rejections, baseline-unchanged) + 9 key-alignment (SOL-104: reorder survival, shuffled-row attribution, both orphan sides, duplicate-key strict fail, missing key field, mutual exclusion, validations) + 1 orphans-HTML render (SOL-104 follow-on) + 4 comparison-layer hardening (4-Jul sweep: identity-guard + key-alignment injection-quote tolerance, ragged-ML-row structural error, zero-data-rows contract). Subprocess-based plus module-level direct-API tests. Wall-clock ~40–110s. | Reconcile logic changes, CLI argparse changes, new validation rules, new exit-code paths, mismatch dict shape changes, HTML report format changes, identity/key canonicalisation changes |
