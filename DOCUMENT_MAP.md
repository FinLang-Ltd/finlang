...
| `src/finlang/tools/reconcile.py` | Independent ML validation layer (SOL-040, v0.7.8): positional alignment, field comparison, audit linkage from `audit.json`, JSON+CSV artifact generation, optional HTML report dispatch (when `emit_html=True` and `output_dir` set). Decoupled from engine — post-engine hook called from `run_finlang.py`. | New reconcile flags, alignment modes (Phase 2: key-based), report schema changes, audit-extraction logic changes |
