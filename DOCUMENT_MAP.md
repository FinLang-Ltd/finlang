...
*Last updated: 29 June 2026 | v0.8.0-target (Unreleased — Path C in progress) | reconcile-v2 (SOL-103 identity guard `--reconcile-identity-fields`, SOL-104 key alignment `--reconcile-key` + orphan artefacts) + impact analysis (SOL-105 `--impact-rules`) + HTTP API parity (`/reconcile` identity/key params + orphan CSVs, new `POST /impact`). Daily quick_check 168/10 (Gate 10 = reconcile + impact, 50 tests); standalone API gate 24. Builds on v0.7.9 SOL-041 FastAPI wrapper (shipped 18 May 2026).*
