...
3. **Tools** — Discovery, rule generation, integrity verification, ML reconciliation, and rule-change impact analysis (`discover.py`, `suggest.py`, `verify.py`, `reconcile.py`, `impact.py`)
4. **Documentation & Rulepacks** — DSL specification, bundled categorisation packs, and user guides
5. **Validation Infrastructure** — 204-test daily suite (10 gates) + rulepack linter, integrity verification, cleanroom PyPI validation, golden master baselines, ML reconciliation (JSON+CSV+HTML), rule-change impact analysis (JSON+CSV+HTML)
...
| `src/finlang/tools/verify.py` | Post-engine integrity verification: SHA-256 fingerprinting, fast/full modes, artifact generation. **Vectorised since SOL-110 (Jul 2026):** `_read_immutable_fields_df` pandas fast path with automatic fallback to the scalar `_read_immutable_fields` (exact-duplicate headers, empty files, any parse failure); `_map_uniques` applies the scalar normalisers once per unique value — **the scalar normalisers stay the single source of semantic truth, never fork them** (`reconcile.py` imports them too, and `_normalize_amount_string` is a synced copy of `run_finlang._to_number` — see "Amount parsing change"); `_sha16` is module-level so tests can force hash collisions. Byte-identical output is pinned by Gate 8's equivalence tests. | Verification logic changes, new verification modes, reader/normaliser changes (re-run the Gate 8 pin tests — they compare against SHA-pinned pre-vectorisation artefacts) |
| `src/finlang/tools/reconcile.py` | Independent ML validation layer (SOL-040, v0.7.8; extended SOL-103/104): positional alignment with optional identity guard (`--reconcile-identity-fields` — positional verification of identity fields, `IdentityMismatchError` → exit 1, `reconcile_identity_failures.{csv,json}`), key-based alignment (`--reconcile-key` — canonicalised composite keys, four populations, orphan CSVs, strict duplicate-key fail), field comparison, audit linkage from `audit.json`, JSON+CSV artifact generation, optional HTML report dispatch (when `emit_html=True` and `output_dir` set). Field canonicalisation reuses `verify.py`'s normalisers (no forked copies). Decoupled from engine — post-engine hook called from `run_finlang.py`. | New reconcile flags, alignment modes, report schema changes, audit-extraction logic changes, identity/key canonicalisation contract changes |
...
| `src/finlang/tools/impact.py` | Rule-change impact analysis (SOL-105 Branches 1–2): one frame, two in-process `run_audit` passes (baseline `--rules` vs candidate `--impact-rules`), vectorised diff on category/flags(set)/status, attribution via last-rule-wins audit linkage, aggregates (transition matrix with indicative absolute amounts, per-rule match deltas, flag/status deltas), `impact_report.json` (schema `impact/1`, rule-text SHA-256s over combined rules minus provenance markers), `impact_changes.csv` (row-level, sorted by row_number). Returns data; CLI prints `format_summary`. Float disclaimer ships in summary + JSON `amount_note`. | New impact flags, diff-class semantics, JSON schema version bumps, CSV column changes, aggregation changes |
| `src/finlang/tools/impact_html.py` | Self-contained HTML impact report (SOL-105 Branch 3): amber REVIEW REQUIRED / green NO BEHAVIOURAL CHANGE banner, headline cards, transition matrix, per-rule deltas, attribution-only section, row-level table capped at **50** (visual-contract decision, down from the spec's 1,000). Mirrors `reconcile_html.py` disciplines: module-level CSS, `html.escape()` on every user string, O(N) list-join, no JS, self-contained. Lazy-imported by `impact.py` when `emit_html=True`. | Visual contract (mockup at `scratch/mockups/impact_html_mockup.html`), CSS theme, new `ImpactResult` fields, row-cap changes |
| `src/finlang/tools/verify_html.py` | Self-contained HTML integrity report (SOL-111): PASS / REVIEW REQUIRED banner, then a **plain-English "What happened" section stating the run's facts up front** (rows read, how ambiguous dates/amounts were parsed, which locale overrides were supplied — or explicitly that none were), scope table (immutable fields compared vs categorisation fields deliberately not — rendered from `verify.py`'s `IMMUTABLE_FIELDS`/`CATEGORISATION_FIELDS` constants so it cannot drift), fingerprint proof sample (first `PROOF_SAMPLE_SIZE`=10 in/out hash pairs, abbreviated), mismatch table only when mismatches exist. Mirrors `reconcile_html.py` disciplines: inline CSS, no JS, `html.escape()` on every user string, self-contained. Imported lazily by `run_finlang.py` when `--verify-html` is set. | Visual contract changes, CSS theme, new `VerifyResult` fields, sample-size changes, field-constant changes in `verify.py` |
...
| `src/finlang/api/main.py` | FastAPI wrapper around the FinLang CLI (SOL-041; extended for reconcile-v2 + impact at v0.8.0): thin REST surface (`GET /`, `GET /health`, `POST /process`, `POST /discover`, `POST /suggest`, `POST /reconcile`, `POST /impact`) over the published CLI entry points via subprocess. Never imports engine internals. `/reconcile` now also exposes `reconcile_identity_fields` + `reconcile_key` Form params and surfaces `orphans_finlang_csv` / `orphans_ml_csv` in the response (key mode). `/impact` mirrors `/reconcile`: baseline `rules` + candidate `impact_rules` uploads, `impact_html`, `?format=html`. Alignment endpoints (`/reconcile`, `/impact`) use `_alignment_http_error`: whitelist `{0,3}` → 200 (review-needed is expected); exit `1` (overloaded — dominant case is client-data misalignment/duplicate keys, rarely I/O) → **422** (not 500) mapped by dominant meaning; `2` → 422. The 422 body is a discriminator: `error` enum (`alignment_error`/`validation_error`) + `exit_code` + `message` + authoritative `stderr`. `?format=html` returns the report directly with `Content-Type: text/html`. Optional API-key auth via `FINLANG_API_KEY` + `X-API-Key`. Curated, not auto-forwarding — new CLI flags need explicit `Form()` parameters. | New endpoints, new form-field parameters, subprocess dispatch logic changes, auth or limit configuration changes, new response models |
...
| `docs/reconciliation.md` | User-facing `--reconcile` feature doc (SOL-040, v0.7.8): when-to-use / when-NOT-to-use, two-pipeline flow diagram, 📍 Cayman worked example with HTML report close, 🎛️ Variations sub-section (minimal/JSON+CSV-no-HTML/multi-field/verify+reconcile combined), CLI usage, output anatomy, exit codes, limitations, roadmap. Voice register matches `workflows.md` / `growth_loop_best_practices.md`. | New reconcile flags, alignment-mode changes, exit-code semantics changes, HTML report format changes, worked-example refresh |
| `docs/verify.md` | User-facing `--verify` / `--verify-full` integrity-verification doc (v0.7.7 backfill, shipped v0.7.8): the integrity-primitive framing (verify shows the engine didn't corrupt the data), single-pipeline-with-snapshots ASCII flow, two-CSV asymmetry callout, Verify-Reconcile-Audit triad closer. Voice register matches `reconciliation.md`. | New verify flags, verification logic changes, output schema changes |
| `docs/api.md` | User-facing HTTP API doc (SOL-041): when-to-use / when-NOT-to-use, request flow diagram, worked example with curl + JSON response, endpoints overview, auth + configuration (env vars), HTTP-status ↔ exit-code mapping, `?format=html` shortcut callout for human inspection, limitations, roadmap, related docs. Voice register matches `workflows.md` / `reconciliation.md`. | New endpoints, new form-field parameters surfaced to users, changes to auth/configuration env vars, exit-code mapping changes |
| `docs/api_reference.md` | Full API reference (SOL-041): form-field tables per endpoint, response schemas (Pydantic shape), HTTP status code mapping, curl recipes for each endpoint (incl. `?format=html` direct-HTML-download variant), deployment notes (reverse-proxy/TLS caveat), determinism contract section. The technical-reference companion to `docs/api.md`. | New endpoints, form-field changes, response shape changes, new env vars, deployment-pattern documentation |
...
| File | Contains | Update When |
|------|----------|-------------|
| `quick_check.ps1` | Daily gate runner (10 gates, 204 tests + rulepack linter), single-line-per-gate display. Gate 10 ("Validation: reconcile + impact") runs `test_reconcile.py` + `test_impact.py` together. Wall-clock ~120–170s typical (varies with cache state). | New daily gates, test count changes |
...
| `run_showcase_local.cmd` | Double-click launcher for showcase against `{workspace}\prod` local source — pre-release rehearsal mode (NOT a PyPI validation; use only when v0.7.x+1 not yet published). Wrapper runs Phase 0–4 (Setup → Install → Tests → Demo → Teardown); reconciliation lives in `finlang_demo_v4.ps1` STEP 9 with a version guard that Yellow-skips if installed FinLang < 0.7.8 (post-demo-restructure 14 May 2026). | Rarely |
...
| `test_rule_interactions.py` | 27 engine | Cache invalidation, exclude lifecycle, flag accumulation, audit modes, idempotency; 4-Jul hardening: audit `changes` key-order canonical across processes, lite-cap counts logged entries, wildcard `\Z` anchor, `list_rulepacks` `.fin` filter | Engine state-transition changes |
...
| `test_verify.py` | 29 verify (incl. 11 SOL-111 `--verify-html` report tests) | Integrity verification: --verify (fast), --verify-full, --verify-output-dir, tampered output detection; 4-Jul hardening: injection-quote round-trip (fast+full), blank-amount no-crash, round-trip invariant x2 (both FINLANG_SAFE_TEXT modes); SOL-110 vectorisation: pinned byte-equivalence, scalar/vector unquote property, hash-collision guard, reader equivalence + path selection, fast-path liveness (Codex P1) | Verification logic or CLI integration changes |
| `test_reconcile.py` | 43 reconcile (Gate 10) | `--reconcile` ML validation: 12 happy/edge tests + 3 validation rejection tests + 1 verify+reconcile coexistence regression + 1 memo-enrichment dict assertion + 1 HTML render+escape end-to-end + 1 dual-flag validation + 5 identity-guard (SOL-103: pass-through w/ normalised amounts, misalignment exit 1 + artefacts + suppression, validation rejections, baseline-unchanged) + 9 key-alignment (SOL-104: reorder survival, shuffled-row attribution, both orphan sides, duplicate-key strict fail, missing key field, mutual exclusion, validations) + 1 orphans-HTML render (SOL-104 follow-on) + 4 comparison-layer hardening (4-Jul sweep: identity-guard + key-alignment injection-quote tolerance, ragged-ML-row structural error, zero-data-rows contract) + 5 ML-date-convention (26-Jul: inference regression, assumed-mode recorded, explicit-flag recorded, unparseable explicit format = structural exit 1, inert positional combination = exit 2). Subprocess-based plus module-level direct-API tests. Wall-clock ~40–110s. | Reconcile logic changes, CLI argparse changes, new validation rules, new exit-code paths, mismatch dict shape changes, HTML report format changes, identity/key canonicalisation changes |
| `test_impact.py` | 17 impact (Gate 10, folded with `test_reconcile.py` — gate-wired 28 Jun 2026 per explicit Rule-4 approval; gate count stays 10) | SOL-105 impact analysis: identical packs → exit 0; category/flag/status changes → behavioural exit 3; rename + shadow → attribution-only exit 0; rule removal; hand-computed amount aggregation (150.00); exit-code contract; pass isolation (leak-detector rule); flag validations (incl. `--impact-html` requires `--impact-rules`); JSON schema + hash-equality semantics; CSV column contract + sort + change_class; **HTML render + escape + float disclaimer (test 14), 50-row cap + CSV-keeps-all (test 15), green banner on no-change (test 16)**; 4-Jul hardening: attribution correct across dropped rows (test 17). Run via `python -m pytest test_impact.py` from `test-suite/`. Wall-clock ~30s. | Impact diff semantics, JSON schema changes, CSV column changes, new impact flags, HTML report changes, exit-code mapping changes |
| `test_api.py` | 29 API (standalone gate — NOT part of `quick_check.ps1`'s 204/10) | SOL-041 FastAPI wrapper integration tests via `fastapi.testclient.TestClient`: health, root, `/process`, `/discover`, `/suggest`, `/reconcile` (perfect/mismatch → 200, HTML, `format=html`); **reconcile-v2: `reconcile_key` orphans surfaced, `reconcile_identity_fields` pass-through; `/impact`: no-change→exit 0, behavioural→exit 3, `format=html`, `impact_html` validation, baseline-required**; CLI/API reconcile parity, auth gating; **4-Jul hardening: verify-fail 422 carries verify_report, empty FINLANG_API_KEY = auth disabled (constant-time compare)**. Run via `python -m pytest test_api.py -v` from `test-suite/` with `pip install -e ../prod[api]` + `pip install httpx` in `.venv-quickcheck`. Wall-clock ~28s. | New API endpoints, form-field changes, response-shape changes, auth behaviour changes, exit-code mapping changes |
...
|----------|----------------|
| **New CLI flag** | `run_finlang.py` (argparse), `cli_reference.md`, `flags.md`, `--help` auto-updates, smoke tests if testable. **If it lands on an API-exposed command — see "API surface parity" below.** |
| **New discover/suggest flag** | `discover.py` or `suggest.py` (argparse), `cli_reference.md`, `flags.md`, `test_discover_suggest.py` if testable. **`/discover` + `/suggest` are API endpoints — see "API surface parity" below.** |
| **API surface parity** (any new/changed CLI capability) | The FastAPI wrapper is **curated, not auto-forwarding** — new CLI flags/commands do NOT reach the API automatically. For anything exposed via `/process`, `/discover`, `/suggest`, `/reconcile`, `/impact`: add the `Form()` param (or endpoint) to `src/finlang/api/main.py`, a `test_api.py` case, and `docs/api_reference.md` + `docs/api.md`; run the API count sweep (`test_api.py` total in `prod/CLAUDE.md` + `prod/AGENTS.md` canary headers, `README.md`, this file). **If a feature is deliberately CLI-only for a release, log that decision in the release notes — silence reads as an accidental gap.** Surfaced when v0.8.0 reconcile-v2 (SOL-103/104) + impact (SOL-105) shipped to the CLI but the API lagged a full sprint — caught only by a late "is this in the API?" check and fixed before release. Ensuring the API is up to date is core to every feature, not a fast-follow. |
...
| **New documentation file** (e.g. `docs/<topic>.md`) | The new doc file itself, `DOCUMENT_MAP.md` (Documentation table — add a row with role + Update-When), `README.md` (Documentation section — add a cross-link), `docs/install.md` (What's Next links — add a bullet), `docs/cli_reference.md` (Related Documentation — add if CLI-adjacent). Surfaced when v0.7.7 `verify.md` and v0.7.8 `reconciliation.md` shipped without these rows; the install.md "What's Next" missed both for two releases. |
| **New `pyproject.toml` extras group** (e.g. `[api]`, `[bench]`, future `[test]`) | `pyproject.toml` (declare the group), `DOCUMENT_MAP.md` (Core Source Files row for `pyproject.toml` Update-When already covers this; verify it does), `README.md` (Installation section — add `pip install "finlang[<group>]"` block), `docs/install.md` (Quick Install section — add the same block). Surfaced when v0.7.9 `[api]` extras shipped — README was updated but install.md was missed for 5 days. |
