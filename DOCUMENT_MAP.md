...
*Last updated: 20 July 2026 | v0.8.2 (shipped 20 Jul 2026, SOL-110 verify vectorisation) | reconcile-v2 (SOL-103 identity guard `--reconcile-identity-fields`, SOL-104 key alignment `--reconcile-key` + orphan artefacts) + impact analysis (SOL-105 `--impact-rules`) + HTTP API parity (`/reconcile` identity/key params + orphan CSVs, new `POST /impact`). Daily quick_check 187/10 (Gate 8 = verify, 18 tests; Gate 10 = reconcile + impact, 55 tests); standalone API gate 26. Builds on v0.7.9 SOL-041 FastAPI wrapper (shipped 18 May 2026).*
