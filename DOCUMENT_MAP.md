...
*Last updated: 17 May 2026 | v0.7.9-target (Unreleased) | SOL-041 FastAPI wrapper: new `src/finlang/api/` package (main.py + __init__.py with `?format=html` query param on `/reconcile`), `[api]` optional extras group + `finlang-api` console script in pyproject.toml, new docs `docs/api.md` + `docs/api_reference.md`, standalone 17-test gate `test-suite/test_api.py`. Demo restructure: reconciliation moved out of showcase wrapper Phase 3 into `finlang_demo_v4.ps1` STEP 9 (closing punchline before STEP 10 USE CASES); wrapper renumbered Phase 0–4. Daily quick_check 137/10 unchanged (standalone gate decision). Builds on v0.7.8 SOL-040 reconcile (shipped 9 May 2026).*
...
5. **Validation Infrastructure** — 168-test daily suite (10 gates) + rulepack linter, integrity verification, cleanroom PyPI validation, golden master baselines, ML reconciliation (JSON+CSV+HTML), rule-change impact analysis (JSON+CSV+HTML)
...
| File | Contains | Update When |
|------|----------|-------------|
| `quick_check.ps1` | Daily gate runner (10 gates, 168 tests + rulepack linter), single-line-per-gate display. Gate 10 ("Validation: reconcile + impact") runs `test_reconcile.py` + `test_impact.py` together. Wall-clock ~120–170s typical (varies with cache state). | New daily gates, test count changes |
...
| `test_reconcile.py` | 34 reconcile (Gate 10) | `--reconcile` ML validation: 12 happy/edge tests + 3 validation rejection tests + 1 verify+reconcile coexistence regression + 1 memo-enrichment dict assertion + 1 HTML render+escape end-to-end + 1 dual-flag validation + 5 identity-guard (SOL-103: pass-through w/ normalised amounts, misalignment exit 1 + artefacts + suppression, validation rejections, baseline-unchanged) + 9 key-alignment (SOL-104: reorder survival, shuffled-row attribution, both orphan sides, duplicate-key strict fail, missing key field, mutual exclusion, validations) + 1 orphans-HTML render (SOL-104 follow-on, test 34). Subprocess-based plus 2 module-level direct-API tests. Wall-clock ~40–110s. | Reconcile logic changes, CLI argparse changes, new validation rules, new exit-code paths, mismatch dict shape changes, HTML report format changes, identity/key canonicalisation changes |
| `test_impact.py` | 16 impact (Gate 10, folded with `test_reconcile.py` — gate-wired 28 Jun 2026 per explicit Rule-4 approval; gate count stays 10) | SOL-105 impact analysis: identical packs → exit 0; category/flag/status changes → behavioural exit 3; rename + shadow → attribution-only exit 0; rule removal; hand-computed amount aggregation (150.00); exit-code contract; pass isolation (leak-detector rule); flag validations (incl. `--impact-html` requires `--impact-rules`); JSON schema + hash-equality semantics; CSV column contract + sort + change_class; **HTML render + escape + float disclaimer (test 14), 50-row cap + CSV-keeps-all (test 15), green banner on no-change (test 16)**. Run via `python -m pytest test_impact.py` from `test-suite/`. Wall-clock ~30s. | Impact diff semantics, JSON schema changes, CSV column changes, new impact flags, HTML report changes, exit-code mapping changes |
| `test_api.py` | 24 API (standalone gate — NOT part of `quick_check.ps1`'s 168/10) | SOL-041 FastAPI wrapper integration tests via `fastapi.testclient.TestClient`: health, root, `/process`, `/discover`, `/suggest`, `/reconcile` (perfect/mismatch → 200, HTML, `format=html`); **reconcile-v2: `reconcile_key` orphans surfaced, `reconcile_identity_fields` pass-through; `/impact`: no-change→exit 0, behavioural→exit 3, `format=html`, `impact_html` validation, baseline-required**; CLI/API reconcile parity, auth gating. Run via `python -m pytest test_api.py -v` from `test-suite/` with `pip install -e ../prod[api]` + `pip install httpx` in `.venv-quickcheck`. Wall-clock ~28s. | New API endpoints, form-field changes, response-shape changes, auth behaviour changes, exit-code mapping changes |
