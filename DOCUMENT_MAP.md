...
*Last updated: 4 July 2026 | v0.8.0-target (Unreleased — Path C in progress) | reconcile-v2 (SOL-103 identity guard `--reconcile-identity-fields`, SOL-104 key alignment `--reconcile-key` + orphan artefacts) + impact analysis (SOL-105 `--impact-rules`) + HTTP API parity (`/reconcile` identity/key params + orphan CSVs, new `POST /impact`). Daily quick_check 182/10 (Gate 10 = reconcile + impact, 55 tests); standalone API gate 26. Builds on v0.7.9 SOL-041 FastAPI wrapper (shipped 18 May 2026).*
...
5. **Validation Infrastructure** — 182-test daily suite (10 gates) + rulepack linter, integrity verification, cleanroom PyPI validation, golden master baselines, ML reconciliation (JSON+CSV+HTML), rule-change impact analysis (JSON+CSV+HTML)
...
| File | Contains | Update When |
|------|----------|-------------|
| `quick_check.ps1` | Daily gate runner (10 gates, 182 tests + rulepack linter), single-line-per-gate display. Gate 10 ("Validation: reconcile + impact") runs `test_reconcile.py` + `test_impact.py` together. Wall-clock ~120–170s typical (varies with cache state). | New daily gates, test count changes |
...
| `test_verify.py` | 13 verify | Integrity verification: --verify (fast), --verify-full, --verify-output-dir, tampered output detection; 4-Jul hardening: injection-quote round-trip (fast+full), blank-amount no-crash, round-trip invariant x2 (both FINLANG_SAFE_TEXT modes) | Verification logic or CLI integration changes |
